...
from i18n import t as tr, set_lang, get_lang
...
def _file_open_dlg(title=tr("dlg.open_save")):
    try:
        ...
        p = filedialog.askopenfilename(
            title=title,
            filetypes=[(tr("dlg.filetype"), "*.json"), (tr("dlg.all_files"), "*.*")],
...
def _file_save_dlg(title=tr("dlg.save_title")):
    try:
        import tkinter as tk
        from tkinter import filedialog
        root = tk.Tk(); root.withdraw(); root.attributes("-topmost", True)
        ...
        p = filedialog.asksaveasfilename(
            ...
            filetypes=[(tr("dlg.filetype"), "*.json")],
...
class MainMenuScene:
    ...
    def _build_btns(self):
        cx = W // 2
        self.btns = [
            Btn((cx-120, 310, 240, 60), tr("menu.new"),  "new",  ACTIVE),
            Btn((cx-120, 390, 240, 60), tr("menu.load"), "load", NORMAL),
            Btn((cx-120, 470, 240, 60), tr("menu.help"), "help", NORMAL),
            Btn((cx-120, 550, 240, 60), tr("menu.quit"), "quit", (70, 25, 25)),
            Btn((W-140, H-50, 120, 34), tr("menu.lang"), "lang", (40, 65, 100)),
        ]
    def run(self):
        while True:
            if get_lang() != self._lang:
                self._lang = get_lang(); self._build_btns()
                    ...
                    for b in self.btns:
                        if b.hit(ev.pos):
                            if b.v == "new": return "new"
                                ...
                                self._build_btns()
            self.screen.fill(BG)
            title_s = _ft(52, True).render("Manila", True, GOLD)
            self.screen.blit(title_s, title_s.get_rect(centerx=W//2, top=130))
            sub = _ft(18).render(tr("menu.subtitle"), True, DIM)
            ...
            ver = _ft(12).render(tr("menu.version"), True, DIM)
...
class LogViewer:
    ...
    def draw(self, screen, game_log):
        ...
        t = _ft(22, True).render(tr("logview.title"), True, GOLD)
        ...
        cs = _ft(14).render(tr("logview.close"), True, TEXT)
...
class SetupScene:
    NAMES=["setup.name_1","setup.name_2","setup.name_3","setup.name_4","setup.name_5"]
    # 布局：3列×2行，每玩家槽宽386px
    ...
    def __init__(self,screen):
        ...
        self.inputs=[TextInput(self._inp_rect(i),tr(self.NAMES[i])) for i in range(5)]
    ...
    def _rebuild(self):
        ...
        for i,c in enumerate([3,4,5]):
            self.btns.append(Btn((460+i*130,148,100,46),tr("setup.n_players",n=c),("n",c),ACTIVE if self.n==c else NORMAL))
        # AI/真人切换按钮
        for i in range(self.n):
            col=i//2; row=i%2
            x=self._CX[col]+315; y=self._RY[row]+5
            a=self.ai_flags[i]
            self.btns.append(Btn((x,y,65,36),tr("setup.ai") if a else tr("setup.human"),("ai",i),DIM if a else (50,120,200)))
        # 开始按钮
        self.btns.append(Btn((W//2-95,H-110,190,54),tr("setup.start"),("start",),ACTIVE))
    ...
    def run(self):
        ...
        while True:
                    ...
                    for b in self.btns:
                        if b.hit(ev.pos):
                            ...
                            if k=="n": self.n=b.v[1]; self._rebuild()
                                ...
                                ps=[]
                                for i in range(self.n):
                                    nm=self.inputs[i].text.strip() or tr(self.NAMES[i])
                                    if self.ai_flags[i]:
            ...
            for i in range(self.n):
                ...
                # 标签
                nt=_ft(15).render(tr("setup.player_n",n=i+1),True,TEXT)
...
class GameScene:
    # raw tuples: (x,y,w,h, i18n_key, value, color)
    _META_RAW = [
        (W-336, 4, 76, 28, "game.restart", "restart", (80, 40, 40)),
        (W-252, 4, 76, 28, "game.save",    "save",    (35, 80, 45)),
        (W-168, 4, 76, 28, "game.load",    "load",    (35, 60, 100)),
        (W- 84, 4, 76, 28, "game.log",     "log",     (60, 45, 95)),
    ]
    def __init__(self, screen, players):
        ...
        self._lang = get_lang()
        self._build_meta_btns()
        self.log_viewer = LogViewer()
        self._save_msg = ""; self._save_msg_t = 0

    def _build_meta_btns(self):
        self.meta_btns = [Btn((x,y,w,h), tr(tk), v, c)
                          for x,y,w,h,tk,v,c in self._META_RAW]
    def run(self):
        def _run_game():
            import random
        ...
        while True:
            if get_lang() != self._lang:
                self._lang = get_lang(); self._build_meta_btns()
            mx, my = pygame.mouse.get_pos()
                        ...
                        if mb.hit(ev.pos):
                            ...
                            if mb.v == "save":
                                ...
                                if path:
                                    ...
                                    self._save_msg = tr("game.saved_msg") + os.path.basename(path)
            ...
            if rt == "game_over":
                ...
                # 响应游戏线程，让它正常退出；再 join 确保完全退出后再 reset
                try: bridge.respond(None)
                except Exception: pass
                self.g_thread.join(timeout=3.0)
                return None
            self.renderer.draw(self.screen, ctx, log, current_req, (mx, my))
            self._draw_meta(mx, my); pygame.display.flip(); self.clock.tick(30)
            if not self.g_thread.is_alive() and current_req is None:
                self._game_over_overlay(ctx)
                self.g_thread.join(timeout=1.0)
                return None
    ...
    def _game_over_overlay(self, ctx):
        ...
        t = _ft(30, True).render(tr("gameover.title"), True, GOLD)
        ...
        if players and market:
            ...
            for i, p in enumerate(ranked):
                ...
                s = _ft(18).render(f"{tr('gameover.rank1') if i==0 else str(i+1)+'. '}{p.name}: ¥{nw}", True, c)
                self.screen.blit(s, s.get_rect(centerx=W//2, top=y)); y += 36
        ct = _ft(14).render(tr("gameover.click_exit"), True, DIM)
